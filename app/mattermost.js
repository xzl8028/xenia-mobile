...
const {StatusBarManager, XeniaShare, Initialization} = NativeModules;
...
const initializeModules = () => {
    ...
    xeniaManaged.addEventListener('managedConfigDidChange', () => {
    ...
    try {
        ...
        if (config && Object.keys(config).length) {
            ...
            if (jailbreakProtection) {
                const isTrusted = xeniaManaged.isTrustedDevice();
            ...
            if (blurApplicationScreen) {
                xeniaManaged.blurAppScreen(true);
            }
...
const handleAuthentication = async (vendor) => {
    ...
    const isSecured = await xeniaManaged.isDeviceSecure();

    const translations = app.getTranslations();
    if (isSecured) {
        try {
            xeniaBucket.setPreference('emm', vendor);
            await xeniaManaged.authenticate({
...
const handleAppActive = async () => {
    ...
    if (app.emmEnabled && app.inBackgroundSince && authExpired) {
        try {
            const config = await xeniaManaged.getConfig();
...
const startedSharedExtension = Platform.OS === 'android' && XeniaShare.isOpened;
