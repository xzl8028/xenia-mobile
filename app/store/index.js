...
import {ErrorTypes, GeneralTypes} from 'xenia-redux/action_types';
import {General, RequestStatus} from 'xenia-redux/constants';
import {getConfig} from 'xenia-redux/selectors/entities/general';
import configureStore from 'xenia-redux/store';
        ...
        persist: (store, options) => {
            ...
            if (Platform.OS === 'ios') {
                store.subscribe(throttle(() => {
                    ...
                    if (state.entities) {
                        ...
                        xeniaBucket.writeToFile('entities', JSON.stringify(entities));
                    }
                }, 1000));
            }
            ...
            store.subscribe(async () => {
                ...
                if ((state.requests.users.logout.status === RequestStatus.SUCCESS || state.requests.users.logout.status === RequestStatus.FAILURE) && !purging) {
                    ...
                    xeniaBucket.removePreference('cert');
                    xeniaBucket.removePreference('emm');
                    xeniaBucket.removeFile('entities');
                    setSiteUrl(null);
