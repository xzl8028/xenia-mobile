...
export default class NetworkIndicator extends PureComponent {
    ...
    getNavBarHeight = (isLandscape = this.props.isLandscape) => {
        ...
        } else if (isLandscape && DeviceTypes.IS_TABLET && xeniaManaged.hasSafeAreaInsets) {
    ...
    initializeWebSocket = async () => {
        ...
        if (platform === 'ios') {
            certificate = await xeniaBucket.getPreference('cert');
        }
        ...
        initWebSocket(platform, null, null, null, {certificate, forceConnection: true}).catch(() => {
            ...
            Alert.alert(
                ...
                formatMessage({
                    ...
                    defaultMessage: 'Xenia has encountered an error. Please re-authenticate to start a new session.',
                }),
                [{
                    text: formatMessage({
