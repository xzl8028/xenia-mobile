...
unsigned-ios: stop pre-build check-style ## Build an unsigned version of the iOS app
	...
	@mkdir -p build-ios
	@cd ios/ && xcodebuild -workspace Xenia.xcworkspace/ -scheme Xenia -sdk iphoneos -configuration Release -parallelizeTargets -resultBundlePath ../build-ios/result -derivedDataPath ../build-ios/ CODE_SIGN_IDENTITY="" CODE_SIGNING_REQUIRED=NO
	@cd build-ios/ && mkdir -p Payload && cp -R Build/Products/Release-iphoneos/Xenia.app Payload/ && zip -r Xenia-unsigned.ipa Payload/
	@mv build-ios/Xenia-unsigned.ipa .
	@cd fastlane && bundle exec fastlane upload_file_to_s3 file:Xenia-unsigned.ipa os_type:iOS
	@rm -rf build-ios/
	$(call stop_packager)
...
ios-sim-x86_64: stop pre-build check-style ## Build an unsigned x86_64 version of the iOS app for iPhone simulator
	...
	@cd fastlane && NODE_ENV=production bundle exec fastlane ios unsigned
	@mkdir -p build-ios
	@cd ios/ && xcodebuild -workspace Xenia.xcworkspace/ -scheme Xenia -arch x86_64 -sdk iphonesimulator -configuration Release -parallelizeTargets -resultBundlePath ../build-ios/result -derivedDataPath ../build-ios/ ENABLE_BITCODE=NO CODE_SIGN_IDENTITY="" CODE_SIGNING_REQUIRED=NO ENABLE_BITCODE=NO
	@cd build-ios/Build/Products/Release-iphonesimulator/ && zip -r Xenia-simulator-x86_64.app.zip Xenia.app/
	@mv build-ios/Build/Products/Release-iphonesimulator/Xenia-simulator-x86_64.app.zip .
	@rm -rf build-ios/
	@cd fastlane && bundle exec fastlane upload_file_to_s3 file:Xenia-simulator-x86_64.app.zip os_type:iOS
	$(call stop_packager)
...
unsigned-android: stop pre-build check-style prepare-android-build ## Build an unsigned version of the Android app
	...
	@mv android/app/build/outputs/apk/unsigned/app-unsigned-unsigned.apk ./Xenia-unsigned.apk
	@cd fastlane && bundle exec fastlane upload_file_to_s3 file:Xenia-unsigned.apk os_type:Android
	$(call stop_packager)

test: | pre-run check-style ## Runs tests
	@npm test

build-pr: | can-build-pr stop pre-build check-style i18n-extract-ci ## Build a PR from the xenia-mobile repo
	$(call start_packager)
