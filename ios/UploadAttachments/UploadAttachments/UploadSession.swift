...
@objc @objcMembers public class UploadSession: NSObject, URLSessionDataDelegate {
    ...
    public func createPost(identifier: String) {
        ...
        if uploadSessionData.message != "" || uploadSessionData.fileIds.count > 0 {
            ...
            if #available(iOS 12.0, *) {
                os_log(OSLogType.default, "Xenia will post identifier=%{public}@", identifier)
            }
    ...
    public func attachSession(identifier: String, completionHandler: @escaping () -> Void) {
        ...
        if #available(iOS 12.0, *) {
            os_log(OSLogType.default, "Xenia Attached session with completionHandler identifier=%{public}@", identifier)
        }
    ...
    public func createURLSession(identifier: String) -> URLSession {
        ...
        if #available(iOS 12.0, *) {
            os_log(OSLogType.default, "Xenia Session created identifier=%{public}@", identifier)
        }
        return self.session!
    }
    ...
    public func urlSession(_ session: URLSession, dataTask: URLSessionDataTask, didReceive data: Data) {
        ...
        do {
            ...
            if #available(iOS 12.0, *) {
                os_log(OSLogType.default, "Xenia Failed to receive data identifier=%{public}@ error=%{public}", identifier, error.localizedDescription)
            }
    ...
    public func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        if error == nil {
            ...
            if sessionData.fileIds.count == sessionData.totalFiles {
                if #available(iOS 12.0, *) {
                    os_log(OSLogType.default, "Xenia did complete upload identifier=%{public}@", identifier)
                }
    ...
    public func urlSessionDidFinishEvents(forBackgroundURLSession session: URLSession) {
        if #available(iOS 12.0, *) {
            os_log(OSLogType.default, "Xenia urlSessionDidFinishEvents identifier=%{public}@", session.configuration.identifier ?? "no identifier")
        }
        
        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + Double(Int64(1.0*Double(NSEC_PER_SEC))) / Double(NSEC_PER_SEC), execute: {
            if self.completionHandler != nil {
                if #available(iOS 12.0, *) {
                    os_log(OSLogType.default, "Xenia CALLED COMPLETIONHANDLER")
                }
