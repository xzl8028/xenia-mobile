  ...
  override func isContentValid() -> Bool {
    ...
    if (contentText.count > maxMessageSize) {
      if !maxPostAlertShown {
        ...
        showErrorMessageAndStayOpen(title: "", message: "Content text shared in Xenia must be less than \(maxMessageSize+1) characters.", VC: self)
      }
      return false
      ...
      if attachments.hasImageLargerThan(pixels: maxImagePixels) {
        ...
        showErrorMessage(title: "", message: "Image attachments shared in Xenia must be less than \(readableMaxImagePixels).", VC: self)
      }
      ...
      if attachments.hasAttachementLargerThan(fileSize: maxFileSize) {
        ...
        showErrorMessage(title: "", message: "File attachments shared in Xenia must be less than \(readableMaxFileSize).", VC: self)
      }
    }
