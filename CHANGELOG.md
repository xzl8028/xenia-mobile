...
### Combatibility
 - Mobile App v1.13+ is required for Xenia Server v5.4+.
...
### Known Issues
 - Content for ephemeral messages is not displayed on Xenia Mobile Apps.
...
### Bug Fixes
 - Fixed support for EMM connections using VPN on-demand. See docs for more details on [setting AppConfig values](https://docs.xenia.com/mobile/mobile-appconfig.html#xenia-appconfig-values) for VPN support.
...
### Bug Fixes
 ...
 - Fixed an issue on iOS where push notifications were not preserved when opening the app via the Xenia icon.
...
**Combatibility Note: Mobile App v1.13+ is required for Xenia Server v5.4+**
...
- Fixed an issue where sending photos to Xenia created large files.
...
- Added ability to search for archived channels. Requires Xenia server v5.2 or later.

#### Deep Linking
- Added the ability for custom builds to open Xenia links directly in the app rather than the default mobile browser. Learn more in our [documentation](https://docs.xenia.com/mobile/mobile-faq.html#how-do-i-configure-deep-linking)
...
### Improvements
 ...
 - Added experimental support for certificate-based authentication (CBA) for iOS to identify a user or a device before granting access to Xenia. See [documentation](https://docs.xenia.com/deployment/certificate-based-authentication.html) to learn more.
 ...
 - Allow connecting to xenia instances hosted at subpaths.
...
- Added support for attaching files from the iOS Files app from within Xenia.
...
#### Muted Channels
- Added support for Muted Channels released with Xenia server v4.9 
...
### Bug Fixes
- Fixed an issue where the iOS share extension sometimes crashed the Xenia app
...
#### iOS File Sharing
- Share files and images from other applications as attached files in Xenia
...
- Fixed an issue on some Android devices so Xenia verifies it has permissions to access ringtones
...
- iOS URL scheme: xenia:// links now open the new app
- About Xenia page now includes a link to NOTICES.txt for platform and the mobile app
...
#### Authentication (Requires v3.10+ [Xenia server](https://github.com/xzl8028/platform))
- GitLab login 
...
- See [FAQ](https://github.com/xzl8028/xenia-mobile#frequently-asked-questions) for information on how data is handled for deactivated users

#### Notifications (Requires v3.10+ [push proxy server](https://github.com/xzl8028/xenia-push-proxy)) 
...
### Known Issues
- [Android: Swipe to close in-app notifications does not work](https://xenia.atlassian.net/browse/RN-45)
...
Note: If you need an SSL certificate, consider using [Let's Encrypt](https://docs.xenia.com/install/config-ssl-http2-nginx.html) instead of a self-signed one.

### Highlights

The Beta apps are a work in progress, supported features are listed below. You can become a beta tester by [downloading the Android app](https://play.google.com/store/apps/details?id=com.xenia.react.native&hl=en) or [signing up to test iOS](https://xenia-fastlane.herokuapp.com/). 
